	...
	public void Refresh()
	{
		...
		if (system != null)
		{
			for (uint deviceIndex = 0; deviceIndex < connected.Length; deviceIndex++)
			{
				...
				switch (system.GetTrackedDeviceClass(deviceIndex))
				{
					case ETrackedDeviceClass.Controller:
						if (ETrackedControllerRole.RightHand == system.GetControllerRoleForTrackedDeviceIndex(deviceIndex))
						{
							SetTrackedDeviceIndex(0, deviceIndex);
						}

						if (ETrackedControllerRole.LeftHand == system.GetControllerRoleForTrackedDeviceIndex(deviceIndex))
						{
							SetTrackedDeviceIndex(1, deviceIndex);
						}
						continue;
					case ETrackedDeviceClass.GenericTracker:
						for (int i = 0; i < objects.Length; i++)
						{
							if (GetTrackedDeviceSerialNumber(deviceIndex) == objects[i].GetComponent<SteamVR_TrackedObject>().serialNumber)
							{
								SetTrackedDeviceIndex(i, deviceIndex);
							}
						}
						continue;
				}
		...
		//Disable non-tracked objects
		for (int i = 0; i < objects.Length; i++)
		{
			if ((int) objects[i].GetComponent<SteamVR_TrackedObject>().index == -1)
			{
				SetTrackedDeviceIndex(i, OpenVR.k_unTrackedDeviceIndexInvalid);
			}
		}
	}
}